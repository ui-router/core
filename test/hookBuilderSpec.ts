...
describe('HookBuilder:', function() {
  ...
  beforeEach(function() {
    ...
    const path = [new PathNode(root), new PathNode(A)];
    ...
    const B = $state.target('A.B', null).$state();
    const C = $state.target('A.B.C', null).$state();
    const fromPath = [new PathNode(root), new PathNode(A), new PathNode(B), new PathNode(C)];
  ...
  describe('built TransitionHooks', function() {
    ...
    describe('should have the correct state context', function() {
      const hookTypeByName = name => $trans._pluginapi._getEvents().filter(type => type.name === name)[0];

      const context = hook => hook.stateContext && hook.stateContext.name;
